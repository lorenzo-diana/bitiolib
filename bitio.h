...
/* In read mode, open a memory buffer to read data from it.
 * In write mode, create a memory buffer and write data on it.
 * In the latter case when bit_close() is called buf is updated
 * to point to the buffer created and len is updated to contain
 * the actual len of the buffer.
 * 
...
/*	Write to the memory buffer the data in the internal buffer. If the number
 *	of bits in the buffer is not a multiple of 8 the least #bit % 8 bits
 *	will remain in the buffer.
 *
 *	@param	b, pointer to the struct that rappresent a bitio memory buffer.
 *
 *	@return	On success an integer that rappresent the actual number of
 *			bits writed in the memory buffer, on failure -1 is returned.
 */
